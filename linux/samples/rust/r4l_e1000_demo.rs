...
impl net::DeviceOperations for NetDevice {
    ...
    fn stop(dev: &net::Device, data: &NetDevicePrvData) -> Result {
        ...
        let rctl :u32 = data.e1000_hw_ops.mem_addr.readl(E1000_RCTL)?;
        data.e1000_hw_ops.mem_addr.writel(rctl & !E1000_RCTL_EN, E1000_RCTL)?;

        dev.netif_tx_disable();

        // disable transmits in the hardware
        let tctl :u32 = data.e1000_hw_ops.mem_addr.readl(E1000_TCTL).unwrap() & !E1000_TCTL_EN;
        data.e1000_hw_ops.mem_addr.writel(tctl, E1000_TCTL)?;
        // flush both disables and wait for them to finish
        //data.e1000_hw_ops.e1000_write_flush();
        coarse_sleep(core::time::Duration::from_millis(10));
        ...
        data.napi.disable();
        // irq_disable
        data.e1000_hw_ops.mem_addr.writel(!(0 as u32), E1000_IMC)?;
        //data.e1000_hw_ops.e1000_write_flush();

        // e1000_down_and_stop()
        //    cancel_delayed_work_sync ignored

        // e1000_reset()
        //   call E1000Ops::e1000_reset_hw() only
        //data.e1000_hw_ops.e1000_reset_hw();

        // e1000_free_irq()
        {
            pr_info!("free irq\n");
            let req_reg_box = unsafe {Box::from_raw(data._irq_handler.load(Ordering::Relaxed)) };
            //data._irq_handler.store(core::ptr::null_mut(), Ordering::Relaxed);
        }
    
        {
            // e1000_clean_all_tx_rings()
            pr_info!("clean tx rings\n");
            *data.tx_ring.lock() = None;
            
            // e1000_clean_all_rx_rings()
            pr_info!("clean rx rings\n");
...
impl driver::DeviceRemoval for E1000DrvPrvData {
    fn device_remove(&self) {
        ...
        // field _netdev_reg will be automatically dropped when self is dropped,
        // thus, net::Registration<T>::drop() will be called,
        // thus, unregister_netdev() and free_netdev() will be called.
    }
}
...
impl pci::Driver for E1000Drv {
    ...
    fn remove(dev: &mut pci::Device, data: &Self::Data) {
        ...
        // release mapped resource held by e1000_hw_ops.mem_addr
        // held by _netdev_reg, will be automatically dropped
...
impl Drop for E1000KernelMod {
    fn drop(&mut self) {
        ...
        // when we drop self._dev, pci::Adapter<T>::unregister() will be called
    }
}
